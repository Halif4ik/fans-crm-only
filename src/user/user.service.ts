...
export class UserService {
  ...
  async createUser(createUserDto: CreateUserDto): Promise<User> {
    ...
    if (userFromBd)
      throw new HttpException("User exist in bd", HttpStatus.CONFLICT);
    const hashPassword: string = await bcrypt.hash(createUserDto.password, 5);

    const creatednewUser: User = await this.userRepository.create({
      ...createUserDto,
  ...
  findUserById(userFromGuard: User, id: number): User {
    if (userFromGuard.id === id) return userFromGuard;
