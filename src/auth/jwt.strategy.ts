...
export class JwtStrategyAuth extends PassportStrategy(Strategy, 'jwt-auth') {
  constructor(
    ...
    super({
      ...
      ignoreExpiration:
        configService.get<string>('IGNORE_EXPIRATION_TOKEN') === 'true',
  ...
  async validate(payload: unknown): Promise<User | null> {
    ...
    console.log("strategy-", this.userService.getUserById(jwtBody.id));
    /*add to request user*/
    return this.userService.getUserById(jwtBody.id);
  }
}
